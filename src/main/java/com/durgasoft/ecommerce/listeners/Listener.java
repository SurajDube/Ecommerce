...
public class Listener extends BasePage implements  ITestListener {
	...
	public void onTestSuccess(ITestResult arg0) {
		...
		if(arg0.isSuccess()) {
			...
			File target = new File((String) reportdirectory+ "/passedscreens/" + methodname + "_" + simpledate.format(cal.getTime())+ ".png" );
			try {
	...
	public void onTestFailure(ITestResult arg0) {
		...
		if(!arg0.isSuccess()) {
			File src=((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
			String reportdirectory=(new File(System.getProperty("user.dir")).getAbsolutePath()+"/src/main/java/com/durgasoft/ecommerce");
			File target = new File((String) reportdirectory+"/failedScreens/" +methodname+"_"+simpledate.format(cal.getTime())+ ".png");
			try {
				FileUtils.copyFile(src, target);
			} catch (IOException e) {	
				e.printStackTrace();
